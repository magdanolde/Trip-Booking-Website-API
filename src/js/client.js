...
function insertExcursions(data) {
  ...
  const excursionPrototype = document.querySelector(
...
function addExcursions(liEl) {
  liEl
    .querySelector(".excursions__field--submit")
    .addEventListener("click", (event) => {
      ...
      const childPrice = liEl.querySelector(
        ".excursions__childprice"
      ).textContent;
      const adultPrice = liEl.querySelector(
        ".excursions__adultprice"
      ).textContent;
...
function addItem(name, adultNumber, adultPrice, childNumber, childPrice, id) {
  ...
  const item = {
    name: name,
    adultNumber: validedAdultdNumber,
    adultPrice: adultPrice,
    childNumber: validedChildNumber,
...
function createBasketItems() {
  ...
  document.querySelector(".order__total-price-value").textContent =
    getfinalTotal() + " " + "€";
}
...
function validateOrderForm() {
  ...
  submitBtn.addEventListener("click", (e) => {
    ...
    if (errors.length === 0) {
      e.preventDefault();
      if (cart.length > 0) {
        const totalSumSpan = document.querySelector(
        ...
        alert(
          "Jeszcze nie wybrales zadnej wycieczki. Zapoznaj sie z naszym katalogiem"
        );
      }
    }
  });
